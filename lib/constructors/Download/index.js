module.exports = function (app){
   ...
   * 
   */
  Download.prototype.getYtPlayer = function (){
    ...
    if(script_match.length)
      ...
      return this.f_retryAll("@getYtPlayer: couldn't find ytplayer in d.src");

    ytplayer = ytplayer.replace(/<script>/, '');
    ytplayer = ytplayer.replace(/<\/script>/, '');

    if(ytplayer.length < 5000)
      return this.f_retryAll('@getYtPlayer: ytplayer.length < 5000');
  ...
  Download.prototype.fixSignature = function (){
    ...
    if(best_url.indexOf('signature=') !== -1){
      ...
      this.html5player_url = 'http:' + this.d.ytplayer.config.assets.js;

      
      // Initialize the decipherer
      var decipherer = new Decipherer({
        dl: this,
        f_desc: 'download:' + this.v + ' -> decipherer'
      });

      decipherer = app.modules.f_.setup(decipherer);
  ...
  /**
   * Set Download prototype f_ data
   */
