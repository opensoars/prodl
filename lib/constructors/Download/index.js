...
module.exports = function (app){
  ...
  /**
   * @desc Complete the download url ith the deciphered signature.
   * @public
   */
  ...
  /**
   * @desc Streams data served at d.dl_url to the temp folder.
   * @public
   */
  Download.prototype.downloadFile = function (){
    var self = this,
        ...
        temp_loc_mp4 = self.temp_dir + title + '.m4a',
        temp_loc_mp3 = self.temp_dir + title + '.mp3',
        protocol = dl_url.indexOf('https') !== -1 ? 'https' : 'http',
    
    self.d.bytes_received = 0;

    self.d.temp_loc_mp4 = temp_loc_mp4;
    self.d.temp_loc_mp3 = temp_loc_mp3;
  ...
  Download.prototype.convertFile = function (){
    ...
    function dataHandler (data){
      ...
      if(data.indexOf('Overwrite ?') !== -1 || data.indexOf('? [y/N]') !== -1)
        writeToCp('y');
      ...
      && data.indexOf('time=') !== -1){
        ...
        //if(use_log) log(data);
        //if(use_log) log(Math.round(percentage_done));
      }
    ...
    function closeHandler (){
      if(use_log) log('Coversion process closed');
      end();
    }
    ...
    if(app.params.dl_dir.indexOf('D:/') !== -1) writeToCp('D:');
    writeToCp('cd ' + app.params.dl_dir);
  };
