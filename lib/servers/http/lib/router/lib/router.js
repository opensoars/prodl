module.exports = function (app){
  ...
  /**
   * @desc Extract parameters from an url according to a pattern
  ...
  router.delete = function (url, toCall) {
    if (typeof url != 'string') {
      throw 'router.delete needs an url string as its 1st argument';
    }
    if (typeof toCall != 'function') {
      throw 'router.delete needs a callback function as its 2nd argument';
    }

    this.routes.DELETE[url] = toCall;
