...
module.exports = function (app) {
  ...
  function getPercentage (data, dl_length) {
    ...
    if(!ffmpeg_cl_captures && !avconv_cl_captures) {
      return 0;
    }
  ...
  Download.prototype.getDownloadUrl = function () {
    ...
    /*
    //dump(
    //  'adaptive_fmts' + '-' + this.v,
    //  adaptive_fmts
    //);

    var has_mp4_audio;

    adaptive_fmts.type.forEach(function (type) {
    ...
    if (adaptive_fmts.type.indexOf()) {
      console.log(adaptive_fmts.type);
    }

    return this.f_abort('DEBUG');*/

    for (i = 0; i < adaptive_fmts.type.length; i +=1 ) {
      type = adaptive_fmts.type[i];

      if (type.indexOf('audio/mp4') !== -1) {
        audio_found = true;
        matches.push(i);
        audio_matches.push(i);
      }
    ...
    if (audio_found) {
      ...
      return this.f_retryAll('No audio/mp4 found!');
    }

/*    dump(
      'best_match_i-' + this.v + '-' + new Date().getTime(),
      best_match_i
    );

    dump(
      'adaptive_fmts-' + this.v + '-' + new Date().getTime(),
      adaptive_fmts
    );*/
  ...
  /**
   * Complete the download url with the deciphered signature.
   * @public
   */
  Download.prototype.completeUrl = function () {
    if (this.d.deciphered_sig) {
      this.d.dl_url = this.d.best_url + '&signature=' + this.d.deciphered_sig;
    }
    else {
      this.d.dl_url = this.d.best_url;
    }

    this.f_next();
  };

  Download.prototype.checkUrl = function () {
    if (!this.d.best_url) {
      this.f_retryAll('@checkUrl: !this.d.best_url');
    }
    else if (typeof this.d.best_url !== 'string') {
      this.f_retryAll("@checkUrl: typeof this.d.best_url !== 'string'");
    }
    else if (this.d.best_url.length < 50) {
      this.f_retryAll('@checkUrl: this.d.best_url.length < 50');
    }
    else {
      this.f_next();
    }
  ...
  /**
   * Cleans data from the temp folder in case a previous download left data.
   * @public
   */
  Download.prototype.cleanBeforeDl = function () {
    var self = this,
        fs = app.modules.fs,
    ...
    fs.unlink(temp_loc_mp3, function (err){
      if (err) {
        logWarn('@clean: could not delete ' + temp_loc_mp3);
      }
      else {
      ...
      fs.unlink(temp_loc_mp4, function (err) {
        if (err) {
          logWarn('@clean: could not delete ' + temp_loc_mp4);
        }
        else {
  ...
  Download.prototype.downloadFile = function () {
    ...
    app.modules[protocol].get(dl_url, function (res) {
      ...
      res.on('data', function (c) {
        self.d.bytes_received += c.length;
      });
  ...
  Download.prototype.convertFile = function (){
    ...
    function dataHandler(data) {
      ...
      else if (data.indexOf('size=') !== -1 && data.indexOf('time=') !== -1) {
        var conversion_progress = getPercentage(data, self.d.length_seconds);
        self.conversion_progress = conversion_progress;

        log(
          'converFile: ' + self.v + ' ' + Math.round(conversion_progress) + '%'
        );
      }
      else if (data.indexOf('Permission denied') !== -1) {
        return fail('cp.stderr, Permission denied', data);
      } 
      else if (data.indexOf('Invalid data found when processing input') !== -1) {
        return fail('cp.stderr, Invalid data found when processing' +
          ' input. Probably due to a downloaded file with no data', data);
      }
      else if (data.indexOf('Output file #0 does not contain any') !== -1) {
        return fail('cp.stderr,'  +
    ...
    function errorHandler(err) {
      ...
      return fail('@convertFile: cp.on error', err);
    }
    ...
    if (app.params.dl_dir.indexOf('D:/') !== -1) {
      cp.write('D:');
    }
  ...
  Download.prototype.moveFile = function () {
    var self = this,
        final_loc_mp3 = self.dl_dir + '/' + self.title + '.mp3',
        fs = app.modules.fs;
    ...
    fs.readFile(self.d.temp_loc_mp3, function (err, file_data){
      if (err) {
        return self.f_retryAll('@moveFile: readFile error', err);
      }

      fs.writeFile(final_loc_mp3, file_data, function (err){
        if (err) {
          return self.f_retryAll('@moveFile: writeFile error', err);
        }
        self.f_next();
      });
    });
  ...
  Download.prototype.checkFile = function () {
    ...
    app.modules.fs.stat(self.d.final_loc_mp3, function (err, stats) {
      if (err) {
        return self.f_retryAll('@checkFile: fs.stat error', err);
      }
      if (stats.size < 100) {
   ...
   * @public
   */
  Download.prototype.cleanAfterDl = function () {
    var self = this,
        fs = app.modules.fs,
        temp_loc_mp3 = self.d.temp_loc_mp3,
        temp_loc_mp4 = self.d.temp_loc_mp4;
    ...
    fs.unlink(temp_loc_mp3, function (err){
      if (err) {
        return logWarn('@clean: could not delete ' + temp_loc_mp3);
      }
      log('Succesfuly deleted: ' + temp_loc_mp3);
    });
    ...
    fs.unlink(temp_loc_mp4, function (err){
      if (err) {
        return logWarn('@clean: could not delete ' + temp_loc_mp4);
      }
      log('Succesfuly deleted: ' + temp_loc_mp4);
    });
  ...
  Download.prototype.onFinish = function () {
    console.log('FINISH');
    //dump('f_errs-finish', this.f_errs);
    app.downloads.delete(this.id);


    setTimeout(function () {
      app.downloads.add(
        app.modules.f_.setup(
          new app.Download({v: 'NnTg4vzli5s'})
        ).start()
      );
    }, 5000);
  ...
  Download.prototype.onAbort = function () {
    ...
    //dump(this.title + ' ' + this.v, this.f_errs);

    //dump('f_errs-abort', this.f_errs);
    app.downloads.delete(this.id);
  };
   ...
   */
  var f_ready_Download = app.modules.f_.augment(Download, {
    function_flow: [
      'getSource',
      'findYtPlayer',
      'evalYtPlayer',
    ...
    data_namespace: 'd',
    desc: 'download',
    max_tries: app.params.max_tries || {},
    to_log: (app.params.use_log ? ['all'] : ['silent'])
  });

  return f_ready_Download;
};
