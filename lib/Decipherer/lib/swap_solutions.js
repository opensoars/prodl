  ...
  var solutions = {
    ...
    namespaced_var_manip: function (decipher_body, html5player, decipherer){
      ...
      if(!swap_obj_match || !swap_obj_match.length)
        return self.retry('!swap_obj_match || !swap_obj_match.length');

      var swap_obj_name = swap_obj_match[1];

      if(swap_obj_name.charAt(0) === '$') swap_obj_name = '\\' + swap_obj_name;

      var swap_obj = getObjFromCode(swap_obj_name, html5player);

      if(!swap_obj) return self.retry('!swap_obj');
      ...
      if(!method_matches || !method_matches.length)
        return self.retry('!method_matches || !method_matches.length');

      var method_name, method_args, method_body,
          method_regex;

      for(var method_i=0; method_i<method_matches.length; method_i+=1){
        var method_match = method_matches[method_i];
        method_match = method_match.replace(/[\,\{]/, '');

        var method_regex = /(^.+?):function\((.+?)\)\{(.+?)\}/g
          .exec(method_match);


        if(!method_regex || !method_regex.length) 
          return self.retry("!method_regex");
        ...
        var str = ripped_method.str,
            ...
            callParameters = ripped_method.callParameters,
            replacement_strings = ripped_method.replacement_strings,
            call_regexs = ripped_method.call_regexs;

        var method_call, parameter;
        for(var calls_i=0; calls_i<method_calls.length; calls_i+=1){
        ...
        var call_regex, replacement_str;

        for(var callRegex_i=0; callRegex_i<call_regexs.length; callRegex_i+=1){
    ...
    var_manip: function (decipher_body, html5player, decipherer){
      ...
      if(!swap_fn_name || !swap_fn_name.length)
        return self.retry('!swap_fn_name || !swap_fn_name.length');

      var swap_fn = swap_fn_name[1];

      if(swap_fn.charAt(0) === '$') swap_fn = '\\' + swap_fn;
      ...
      for(var i=0, args='('; i<swapper_args.length; i+=1)
        args += swapper_args[i] + ',';
      args = args.slice(0, -1) + ')';
